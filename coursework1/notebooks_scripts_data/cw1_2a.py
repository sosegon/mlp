...
def activation_network(train_data, valid_data, weights_init, hyper, activation1, activation2, activation_name, file_name_pref):
    ...
    log_file_name = "{:s}_{:s}.txt".format(file_name_pref, activation_name)

    write_to_log_file(log_file_name, header, _[1], _[0])

def train_networks(prefix, hyper, initializer="gui"):
    ...
    if initializer == "gni":
        init = GlorotNormalInit(rng=rng)
    elif initializer == "sei":
        init = SELUInit(rng=rng)

    activation_network(train_data, valid_data, init, hyper, SigmoidLayer(), SigmoidLayer(), "sigmoid", prefix)
    activation_network(train_data, valid_data, init, hyper, ReluLayer(), ReluLayer(), "relu", prefix)
    activation_network(train_data, valid_data, init, hyper, LeakyReluLayer(), LeakyReluLayer(), "lrelu", prefix)
    activation_network(train_data, valid_data, init, hyper, ELULayer(), ELULayer(), "elu", prefix)
    activation_network(train_data, valid_data, init, hyper, SELULayer(), SELULayer(), "selu", prefix)
...
train_networks(prefix, [batch_size, learning_rate, num_epochs],  initializer)
